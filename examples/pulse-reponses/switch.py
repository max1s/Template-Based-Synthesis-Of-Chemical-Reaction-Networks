...
def form_crn():
    input1 = InputSpecies("Input1", sympify("-(-50 + t)/(50 * exp((-50 + t)**2/100))"), initial_value=0) # initial value must be one, or output switches on immediately

    POne = Species('POne', initial_max=1, initial_value=1)
    PTwo = Species('PTwo', initial_max=1, initial_value=0)
    PThree = Species('PThree', initial_max=1, initial_value=0.3)
    POneStar = Species('POneStar', initial_max=1, initial_value=0)
    PTwoStar = Species('PTwoStar', initial_max=1, initial_value=1)
    PThreeStar = Species('PThreeStar', initial_max =1, initial_value=0.7)
    ...
    k19 = create_rate_constant('k_19', 1)
    k20 = create_rate_constant('k_20', 0)
    k21 = create_rate_constant('k_21', sa)
    k22 = create_rate_constant('k_22', 0)
    k23 = create_rate_constant('k_23', 0)
